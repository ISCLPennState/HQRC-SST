...
class hqrc(object):
    ...
    def predictSequence(self, input_sequence, perturb=0):
        dynamics_length = self.dynamics_length
        ...
        if True:
            ...
            for t in range(it_pred_length):
                ...
                if t==0:#add random perturbation at the first timestep
                    out += perturb
                out = np.clip(out, 0, 1)
    ...
    def predictIndexes(self, input_sequence, ic_indexes, dt, set_name):
        ...
        perturbations = [0]
        random_numbers = np.random.uniform(-.1,.1,n_tests)
        for i in range(n_tests):
            perturbations.append(random_numbers[i])
        for ic_num in range(n_tests+1):###
            ...
            perturb = perturbations[ic_num]
            prediction, target, prediction_augment, target_augment = self.predictSequence(input_sequence_ic,perturb)
            ...
            if ic_num < 3: plotIterativePrediction(self, set_name, target, prediction, rmse, rmnse, ic_num, dt, target_augment, prediction_augment, self.dynamics_length)
