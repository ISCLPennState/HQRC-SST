...
from scipy.linalg import pinv as scipypinv2 ##
...
class hqrc(object):
    ...
    def __feed_forward(self, input_sequence, predict, use_lastrho, sparse_input):
        ...
        if predict:
            ...
            sparse_state = np.array(sparse_input,dtype=np.float64)
            stacked_state = np.hstack( [sparse_state, aug_state_list, np.ones([input_length, 1])]) #this is the constant bias term 
            #print('stacked state {}; Wout {}'.format(stacked_state.shape, self.W_out.shape))
            #W_out = self.W_out[:stacked_state.shape[1]] ####            
            predict_sequence = sigmoid(stacked_state @ self.W_out + self.b_out) ####
            #predict_sequence = self.W_out.predict(stacked_state)
            #predict_sequence = np.clip(predict_sequence, 0, 1)
        
        return predict_sequence, state_list
    
    
    def __train(self, input_sequence, output_sequence, sparse_sequence):
        ...
        self.W_out = np.random.rand(self.getReservoirSize() + 1+10, Nout)
        self.b_out = np.random.rand(1, Nout)
        # After washing out, use last density matrix to update
        _, state_list = self.__feed_forward(input_sequence, predict=False, use_lastrho=True,sparse_input=sparse_sequence)

        state_list = np.array(state_list, dtype=np.float64)
        sparse_list = np.array(sparse_sequence, dtype=np.float64)
        state_list = np.concatenate((sparse_list,state_list),axis=1)
        ...
        if self.solver == "pinv_naive":
            ...
            No augment data
            """ #Not Using this hence not updated
        ...
        elif self.solver == "logistic":
            X = np.reshape(state_list, [-1, self.getReservoirSize()+sparse_sequence.shape[1]])
            X = np.hstack( [state_list, np.ones([X.shape[0], 1]) ] )
            Y = np.reshape(output_sequence, [output_sequence.shape[0], -1])
            print('TEACHER FORCING ENDED; direct mapping X Y shape', X.shape, Y.shape)
            # def sigmoid(x):
            ...
            #     dL_dY_pred = 2 * (Y_pred - Y) / X.shape[0]
            ...
            # # Final predictions
            # Y_final_pred = sigmoid(np.dot(X, W)+b)
            # print("Y= ",Y[-1])
            # print("Y_pred= ",Y_pred[-1])
            # print("Final Loss:", mse_loss(Y, Y_final_pred))
            ...
            def mse_loss(y_true, y_pred, W, lambda_reg=0.01):
            ...
            for i in range(iterations):
                # Forward pass: compute predictions
                X_norm = batch_norm(X)
                X_drop = dropout(X_norm, drop_prob)
                z = np.dot(X_drop, W) + b
                Y_pred = sigmoid(z)
                ...
                dL_dW = np.dot(X_drop.T, dL_dz) + 2 * lambda_reg * W
                dL_db = np.sum(dL_dz, axis=0, keepdims=True)
            ...
            # Final predictions with the best weights and biases
            Y_final_pred = sigmoid(np.dot(batch_norm(X), best_W) + best_b)
            print("Final Loss:", mse_loss(Y, Y_final_pred, best_W, lambda_reg))

            
        else:
            X, Y = [], []
            # Augment data and using batch normalization
            XTX = np.zeros((self.getReservoirSize() + 1 + sparse_sequence.shape[1], self.getReservoirSize() + 1 + sparse_sequence.shape[1])) # K=(N_out), X = K x (N_total+1)
            XTY = np.zeros((self.getReservoirSize() + 1 + sparse_sequence.shape[1], output_sequence.shape[1])) # Y = K x N_qrc
        ...
        #('Finalizing weights Wout', W_out.shape)
        self.W_out = best_W # N_total+1 x N_qrc
        self.b_out = best_b
        self.n_trainable_parameters = np.size(self.W_out)+np.size(self.b_out)
        self.n_model_parameters = np.size(self.Uops[0]) * self.nqrc + self.n_trainable_parameters
    ...
    def train(self):
        ...
        with open(self.train_data_path, "rb") as file:
            ...
            sparse_input_train = data["train_sparse_input"]
            ...
            sparse_input_train = sparse_input_train[:N_used]
            dt = data["dt"]
            del data
        ...
        sparse_train = sparse_input_train
        #pca = PCA(n_components=self.nqrc)
        #sparse_train = pca.fit_transform(sparse_input_train)
        #sparse_train = pre_scaler_minmax.fit_transform(sparse_train)
        #sparse_train = self.scaler.scaleData(sparse_train,reuse=1)

        #post_scaler = MinMaxScaler(feature_range=(0, 1))
        #sparse_train = post_scaler.fit_transform(sparse_train)
        ...
        #rep_train_input_seq = self.scaler.scaleData(rep_train_input_seq)
        #sparse_train = self.scaler.scaleData(sparse_input_train,reuse=1)
        print("TRAINING: Dynamics prerun...with layer strength={}".format(self.alpha))
        self.__feed_forward(rep_train_input_seq[:dynamics_length], predict=False, use_lastrho=False,sparse_input=sparse_train[:dynamics_length])
        print("\n")
        ...
        print("\n")
        self.__train(rep_train_input_seq[dynamics_length:(dynamics_length + out_length)], train_output_sequence, sparse_train[dynamics_length:(dynamics_length+out_length)])
    ...
    def predictSequence(self, input_sequence, sparse_input):
        dynamics_length = self.dynamics_length
        ...
        prediction_warm_up, state_list = \
            self.__feed_forward(rep_train_input_seq[:dynamics_length], predict=True, use_lastrho=False, sparse_input=sparse_input[:dynamics_length])
        print("\n")
        ...
        if True:
            ...
            for t in range(it_pred_length):
                ...
                #print("sparse_input shape= ", sparse_input.shape)
                sparse_state = np.array(sparse_input[dynamics_length+t],dtype=np.float64).reshape(1,-1)
                #print("sparse_state shape= ", sparse_state.shape)
                stacked_state = np.hstack( [sparse_state, state_aug, np.ones([1, 1])])
                ...
                def sigmoid(x):
                    return 1 / (1 + np.exp(-x))
                out = sigmoid(stacked_state @ self.W_out + self.b_out) ###predicting one step at a time
                #print("prediction at step ",t," = ",out)
                #out = np.clip(out, 0, 1)
                #model = self.W_out
                #out = model.predict(stacked_state)
    ...
    def testingOnTrainingSet(self):
        ...
        with open(self.train_data_path, "rb") as file:
            ...
            train_sparse_input = data["train_sparse_input"]
            del data
            
        rmnse_avg, num_accurate_pred_005_avg, num_accurate_pred_050_avg, error_freq, predictions_all, truths_all, freq_pred, freq_true, sp_true, sp_pred = self.predictIndexes(train_input_sequence, testing_ic_indexes,train_sparse_input, dt, "TRAIN")
    ...
    def testingOnTestingSet(self):
        ...
        with open(self.test_data_path, "rb") as file:
            ...
            test_sparse_input = data["test_sparse_input"]
            dt = data["dt"]
            del data
            
        rmnse_avg, num_accurate_pred_005_avg, num_accurate_pred_050_avg, error_freq, predictions_all, truths_all, freq_pred, freq_true, sp_true, sp_pred = self.predictIndexes(test_input_sequence, testing_ic_indexes, test_sparse_input, dt, "TEST")
        
        for var_name in getNamesInterestingVars():
            exec("self.{:s}_TEST = {:s}".format(var_name, var_name))
        return 0

    def predictIndexes(self, input_sequence, ic_indexes, sparse_input, dt, set_name):
        n_tests = self.n_tests
        ...
        #pca = PCA(n_components=self.nqrc)
        #sparse_input = pca.fit_transform(sparse_input)
        #sparse_input = self.scaler.scaleData(sparse_input,reuse=1)
        predictions_all = []
        truths_all = []
        rmse_all = []
        rmnse_all = []
        ...
        for ic_num in range(n_tests):###
            ...
            sparse_input_ic = sparse_input[ic_idx-self.dynamics_length:ic_idx+self.it_pred_length]
            prediction, target, prediction_augment, target_augment = self.predictSequence(input_sequence_ic, sparse_input_ic)
