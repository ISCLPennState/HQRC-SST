...
def sparse_linear_combine(u,p,sparse):
    if (not np.all((u >= 0) & (u <= 1))):
        print("error, u_comb not in [0,1]")
    v = (1-sparse)*u + sparse*p
    #v = np.clip(v,0,1)
    if not (v <= 1.00001 and v >= -0.00001):
        print("error, v not in [0,1]")
    return v
...
class hqrc(object):
    ...
    def __init__(self, params):
        ...
        #self.sparse_scaler = scaler(self.scaler_tt, self.scaler_trans, self.scaler_ratio)####
        self.noise_level = params["noise_level"]
        self.model_name = self.createModelName(params)

        #initialise sparse input matrix
        # Generate a random matrix with values in [0, 1]
        # w_sparse = np.random.rand(self.nqrc, 10)
        
        # # Normalize each row to ensure the sum is at most 1
        # row_sums = np.sum(w_sparse, axis=1)
        
        # # Avoid division by zero by replacing zero sums with one (though unlikely with rand)
        # row_sums[row_sums == 0] = 1
        
        # # Normalize the rows
        # w_sparse = w_sparse / row_sums[:, np.newaxis]

        # self.W_sparse = w_sparse ####
        self.W_sparse = np.random.normal(0,1,(self.nqrc,self.nqrc)) ####
    ...
    def __step_forward(self, local_rhos, input_val, sparse_input):
        nqrc = self.nqrc
        local_prev_states = []
        for i in range(nqrc):
            Uop = self.Uops[i]
            ...
            if nqrc > 1 and prev_states[0] is not None:
                ...
                value = sparse_linear_combine(value,sparse_input[i],self.sparse)
    ...
    def __feed_forward(self, input_sequence, sparse_sequence, predict, use_lastrho):
        ...
        for time_step in range(0, input_length):
            ...
            sparse_val = sparse_sequence[time_step]
            local_rhos = self.__step_forward(local_rhos, input_val, sparse_val)
        ...
        if predict:
            ...
            sparse_state = np.array(sparse_sequence,dtype=np.float64)
            stacked_state = np.hstack( [sparse_state, aug_state_list, np.ones([input_length, 1])])
    ...
    def __train(self, input_sequence,sparse_sequence, output_sequence):
        ...
        _, state_list = self.__feed_forward(input_sequence,sparse_sequence, predict=False, use_lastrho=True)
    ...
    def train(self):
        ...
        self.__feed_forward(rep_train_input_seq[:dynamics_length],sparse_train[:dynamics_length], predict=False, use_lastrho=False)
        print("\n")
        ...
        print("\n")
        self.__train(rep_train_input_seq[dynamics_length:(dynamics_length + out_length)],sparse_train[dynamics_length:dynamics_length+out_length], train_output_sequence)
    ...
    def predictSequence(self, input_sequence, sparse_input):
        ...
        prediction_warm_up, state_list = \
            self.__feed_forward(rep_train_input_seq[:dynamics_length],sparse_input[:dynamics_length], predict=True, use_lastrho=False)
        print("\n")
        ...
        if True:
            ...
            for t in range(it_pred_length):
                ...
                local_rhos = self.__step_forward(local_rhos, input_val,sparse_input[dynamics_length+t])
            self.last_rhos = local_rhos.copy()
        else:
    ...
    def testingOnTrainingSet(self):
        ...
        rmnse_avg, num_accurate_pred_005_avg, num_accurate_pred_050_avg, error_freq, predictions_all, truths_all, freq_pred, freq_true, sp_true, sp_pred = self.predictIndexes(train_input_sequence, train_sparse_input ,testing_ic_indexes, dt, "TRAIN")
    ...
    def testingOnTestingSet(self):
        ...
        rmnse_avg, num_accurate_pred_005_avg, num_accurate_pred_050_avg, error_freq, predictions_all, truths_all, freq_pred, freq_true, sp_true, sp_pred = self.predictIndexes(test_input_sequence, test_sparse_input, testing_ic_indexes, dt, "TEST")
        
        for var_name in getNamesInterestingVars():
            exec("self.{:s}_TEST = {:s}".format(var_name, var_name))
        return 0

    def predictIndexes(self, input_sequence, sparse_input_sequence, ic_indexes, dt, set_name):
        n_tests = self.n_tests
        ...
        # Normalize the reduced array to the range [0, 1]
        post_scaler = MinMaxScaler(feature_range=(0, 1))
        sparse_input = post_scaler.fit_transform(sparse_input)
