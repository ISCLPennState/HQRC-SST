...
class ZoneoutWrapper(tf.keras.layers.RNNBase):
    """Add Zoneout to a RNN cell."""
    ...
    def zoneoutLoop(self, state, new_state):
        ...
        for i, state_element in enumerate(state):
            random_tensor = 1 - self._zoneout_prob  # keep probability
            random_tensor += tf.random.uniform(tf.shape(state_element), dtype=self._dtype)
            # 0. if [zoneout_prob, 1.0) and 1. if [1.0, 1.0 + zoneout_prob)
            binary_tensor = tf.floor(random_tensor)
            final_new_state[
            i] = (new_state[i] - state_element) * binary_tensor + state_element
    ...
    def call(self, inputs, state, training=None, mask=None):
        ...
        if isinstance(self._cell.state_size, tuple):
            return output, tf.keras.layers.LSTMStateTuple(final_new_state[0], final_new_state[1])
