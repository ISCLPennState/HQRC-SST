...
if __name__  == '__main__':
    ...
    if args.used == 5:
        ...
        samples = ['V=10,alpha=0.1', 'V=15,alpha=0.0', 'V=5,alpha=0.0'] ### 'V=10,alpha=0.0',
        title = 'demo'
    elif args.used == 4:
    ...
    for i in range(ns):
        ...
        bx.set_yticks([])
        bx.set_zticks([])
        bx.xaxis.set_pane_color((0.0, 0.0, 0.0, 0.9))
        bx.yaxis.set_pane_color((0.0, 0.0, 0.0, 0.9))
        bx.zaxis.set_pane_color((0.0, 0.0, 0.0, 0.9))
        bx.grid(False)
        ...
        if len(Wouts.keys()) > 0:
            ...
            for j in range(Wouts[label].shape[0]):
                ...
                sns.kdeplot(Wout, legend=False, fill=True, ax=cx, label='Wo={}'.format(j+1))

            dx = plt.subplot2grid((7, 4), (6,i), colspan=1, rowspan=1)
            for j in range(coeffs[label].shape[0]):
                coef = coeffs[label][j].ravel()
                sns.kdeplot(coef, legend=False, fill=True, ax=dx, label='Wf={}'.format(j+1))
