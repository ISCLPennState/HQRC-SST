...
tf_initializers = {"xavier": tf.initializers.GlorotUniform(), "normal": tf.initializers.TruncatedNormal()}
...
class rnn_statefull(object):
    ...
    def rnn_layer(self, cell_type, name, inputs, input_size, num_hidden, activation_str, ln, initial_hidden_state):
        ...
        with tf.name_scope(scope_name):
            # Create RNN cell
            if cell_type == 'lstm':
                cell = tf.keras.layers.LSTM(units=num_hidden, activation=activation_str, dropout=self.dropout_keep_prob, recurrent_dropout=1-self.zoneout_keep_prob, return_sequences=True, return_state=True)#self._drop->self.drop, basically works if self.drop is always 1
            elif cell_type == 'gru':
                cell = tf.keras.layers.GRU(units=num_hidden, activation=activation_str, dropout=self.dropout_keep_prob, recurrent_dropout=1-self.zoneout_keep_prob, return_sequences=True, return_state=True)#same for self.zoneout
    ...
    def __init__(self, params):
        ...
        #self.dropout_keep_prob = tf.cast(self.dropout_keep_prob, dtype=tf.float32)
        #self.zoneout_keep_prob = tf.cast(self.zoneout_keep_prob, dtype=tf.float32)
        ...
        self.input = tf.keras.Input(shape=[None, self.input_dim], dtype=self.dtype_tf)
        self.target = tf.keras.Input(shape=[None, self.input_dim], dtype=self.dtype_tf)
        self._dropout_keep_prob = tf.keras.Input(shape=(), dtype=self.dtype_tf)
        self._zoneout_keep_prob = tf.keras.Input(shape=(), dtype=self.dtype_tf)
        self._is_training = tf.keras.Input(shape=(), dtype=tf.bool)

        self.initial_hidden_states = []
        for ln in range(self.rnn_num_layers):
            if self.rnn_cell_type == 'lstm':
                c_state = tf.keras.Input(shape=[self.rnn_size_layers], dtype=self.cdtype_tf)
                h_state = tf.keras.Input(shape=[self.rnn_size_layers], dtype=self.cdtype_tf)
                initial_state = tf.keras.layers.LSTMStateTuple(c_state, h_state)
                self.initial_hidden_states.append(initial_state)
            else:
                self.initial_hidden_states.append(tf.keras.Input(shape=[self.rnn_size_layers], dtype=self.cdtype_tf))
